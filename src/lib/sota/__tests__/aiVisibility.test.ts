...
const padding = 'filler one two three four five six seven eight nine ten eleven twelve thirteen fourteen fifteen sixteen seventeen eighteen nineteen twenty';
...
describe('AIVisibility.measureAIVisibility', () => {
  ...
  it('flags thin paragraphs with no signals', () => {
    const html = para(`We believe content needs to be helpful and human and clear. ${padding}`);
    const r = measureAIVisibility(html);
